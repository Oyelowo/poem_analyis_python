...
Note : This module was created to analyse a poem but can be used for other 
        ...
        3. creating a list of time difference(delta) time between words, when reading.
            The time weight can also be increase or decreased by user. This was 
            made flexible to allow for slower or faser readers.
...
#    create an empty list where the time difference will be appendded.
    delta_time_list = []
#    set the initial time of the text. 
    delta_time = 1
#    append the initial time to the empty list.
...
#   also, iterate over all the words in each line and get the indexes and each word.            
...
def words_time_str(text_data, word_time=1, line_time=2, chapter_time=6):
    """
    ...
    Example:
        words_time_str(text_data=data, word_time=1, line_time=2, chapter_time=6)

    """
#    get all the delta time list above
    dt_list = delta_time_list(text_data, word_time, line_time, chapter_time)
#    get the list of all corresponding words
    w_list = words_list_lower(text_data, word_time, line_time, chapter_time)
...
def words_time_dict(text_data, word_time=1, line_time=2, chapter_time=6):
    """
    ...
    Example:
        words_time_dict(text_data=data, word_time=1, line_time=2, chapter_time=6)
    """
#   get all the delta time list above
    dt_list = delta_time_list(text_data, word_time, line_time, chapter_time)
#    get the list of all corresponding words
...
def unsorted_words_df(text_data, word_time=1, line_time=2, chapter_time=6):
    """
    ...
    Example:
        unsorted_words_df(text_data=data, word_time=1, line_time=2, chapter_time=6)
    """
#    get the dictionary which includes all the words and their corresponding delta times.
    wt_dict = words_time_dict(text_data, word_time, line_time, chapter_time)
...
def sorted_words_df(text_data, by='count', rank_words = 100, ascending=False
                 ,myfolder=None,filename='Output', sep=";",file_format=".txt",
                 word_time=1, line_time=2, chapter_time=6):
    """
    Arguments:
        text_data : the data with the texts.
        by(optional) : the variable you are sorting the words \
                        by(only 'count' and'avg_wait_time' allowed).
        rank_words : the number of words you want to rank.
        ascending : True or False, depending if you want the top words or bottom words.
    ...
    Example:
        unsorted_words_df(text_data=data, word_time=1, line_time=2, chapter_time=6)
    """
#    convert the variable for sorting to lowercase to make it not case sensitive.
    by=by.lower()
#    raise error if user input another variable name other than the default names.
...
#        get the unsorted dataframe with words, count and average waiting time.
        words_df = unsorted_words_df(text_data, word_time, line_time, chapter_time)
...
#        if the folder is included as an argument, export the file to the folder
